...
#   https://github.com/KEINOS/TPL-Golang-HelloWorld-MultiArch
...
This script currently does NOT support Docker in Docker. If you are running this script on VSCode via
Remote-Containers then "reopen locally" and run this script again.
...
cd $PATH_DIR_PARENT
docker build \
    --file $PATH_DIR_PARENT/Dockerfile \
    --no-cache \
...
docker run \
    ...
    --entrypoint /bin/mv \
    $NAME_IMAGE \
    -f /$NAME_APP /root/bin/$NAME_BIN_EXPORT
